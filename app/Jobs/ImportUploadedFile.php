	...
	public function handle()
	{
		if (Storage::disk('local')->exists($this->fileWithPath)) {
			...
			foreach ($csv as $offset => $record) {
				...
				if (empty($phone)) {
					$member->receives_text = false;
				}
				if (empty($record['Email'])) {
					$member->receives_email = false;
				}
				$member->save();
				$messageLines[] = "Creted/Updated email and sms_phone for record ".$offset."...".$record['First Name']." ".$record['Last Name'].".";
